...
exports.default = {
    getNextTwoWeeks: () => {
        ...
        for (let i = 1; i < 15; i++) {
            ...
            const day = constants_1.default.DAYS[date.getDay()];
            dates.push({
                date,
                day,
                disable: false,
    ...
    getTimeSlots: (startTime, endTime, break_start, break_end, duration) => {
        const slots = [];
        const start = parseInt(startTime.split(":")[0]) * 60 +
            parseInt(startTime.split(":")[1]);
        const end = parseInt(endTime.split(":")[0]) * 60 + parseInt(endTime.split(":")[1]);
        const breakStart = parseInt(break_start.split(":")[0]) * 60 +
            parseInt(break_start.split(":")[1]);
        const breakEnd = parseInt(break_end.split(":")[0]) * 60 +
        ...
        for (let i = 0; i < slotsCount; i++) {
            ...
            slots.push({
                start: `${startHour < 10 ? "0" + startHour : startHour}:${startMinute < 10 ? "0" + startMinute : startMinute}`,
